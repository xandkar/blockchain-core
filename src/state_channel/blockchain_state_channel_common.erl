        ...
        {packet, Packet} ->
            ...
            case maps:get(PacketHash, PendingOffers, undefined) of
                undefined ->
                    ...
                    blockchain_state_channels_server:packet(Packet, Time, HandlerState#handler_state.handler_mod, self()),
                    {ok, HandlerState};
                {PendingOffer, PendingOfferTime} ->
                    case blockchain_state_channel_packet_v1:validate(Packet, PendingOffer) of
                        ...
                        true ->
                            lager:info("sc_handler server got packet: ~p", [Packet]),
                            blockchain_state_channels_server:packet(Packet, PendingOfferTime, HandlerState#handler_state.handler_mod, self()),
...
handle_offer(Offer, Time, HandlerState) ->
    ...
    case blockchain_state_channels_server:offer(Offer, HandlerState#handler_state.ledger, HandlerState#handler_state.handler_mod, self()) of
        ok ->
